...
#subset 
hippocampus <-sampleInfo[(sampleInfo$group=='AD_Hippocampus'|sampleInfo$group=='control_hippocampus'),]
#create matrix of that pair
mat_hippocampus <- matrix[,hippocampus$ID]

#BEFORE EXCLUDE OUTLIER, RUN DEG FIRST:

# Convert the phenotype data to a design matrix
design <- model.matrix(~0+ group,data=hippocampus)
# Create a linear model and fit it to the gene expression data
fit <- lmFit(mat_hippocampus, design)
# Create contrasts for the groups of interest
contrast.matrix <- makeContrasts(hippocampus_vs_control = groupAD_Hippocampus - groupcontrol_hippocampus, levels=colnames(coef(fit)))
...
ggplot(data = data.frame(full_results), aes(x = logFC, y = -log10(P.Value))) +
  ...
  labs(x = "Log2 Fold Change (AD-hippo vs Control)", y = "-log10(P-Value)",
       title = "Volcano Plot for AD-hippocampus vs Control (GSE5281)") +theme_bw()+
...
gene_matrix <- as.matrix(mat_hippocampus[ids_of_interest,])
annot<-data.frame(group=hippocampus$group,row.names = rownames(hippocampus))
...
pca <- prcomp(t(mat_hippocampus))

## Join the PCs to the sample information
cbind(hippocampus, pca$x) %>% 
...
#write.csv(full_results,file = '5281_hippocampus_pval.csv')
...
mat_AUC<-t(mat_hippocampus[risk_list,])

mat_AUC<-as.data.frame(mat_AUC)
mat_AUC <- mat_AUC[, !colSums(is.na(mat_AUC)) > 0]

label<-as.factor(ifelse(hippocampus$phenotype=='AD',1,0))
...
write.csv(auc_scores,file = '5281_auc_hippocampus_new.csv')
...
mat_all_transpose<-t(mat_hippocampus)
...
km_data_all$group<-hippocampus$phenotype
